...
export const paymentService = {
  ...
  async getByInvoice(invoiceId: string, ctx: GraphQLContext) {
    if (!ctx.user) throw new Error("Not authenticated"); // 🔒 TS fixed
  ...
  async create(input: any, ctx: GraphQLContext) {
  ...
  // 🔹 Update payment
  async update(id: string, input: any, ctx: GraphQLContext) {
    if (!ctx.user) throw new Error("Not authenticated");
